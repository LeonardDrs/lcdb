						...
						<li class="produits"><a href="#" title="Nos produits">Nos produits</a>
							<ul>
								{foreach from=$menu_cats item=cat name=foo}
									<li class="{if $smarty.foreach.foo.first}first {/if}{if $smarty.foreach.foo.last}last {/if}"><a href="{$link->getCategoryLink($cat.id_category, $cat.link_rewrite)|escape:'htmlall':'UTF-8'}" title="{$cat.name}">{$cat.name}</a></li>
								{/foreach}
							</ul>
						</li>
