							...
							<li id="delivery-home-li">
								{if $option.unique_carrier}
									{foreach $option.carrier_list as $carrier}
										<p>
											<label class="radio" for="delivery_option_{$id_address}_{$option@index}"><input{if $carrier.instance->id_reference == $ID_RELAY_CARRIER} class="choose-relay_" idaddress="{$id_address}{/if} type="radio" name="delivery" value="{$carrier.instance->id}" onchange="{if $opc}updateCarrierSelectionAndGift();{else}updateExtraCarrier('{$key}', {$id_address});{/if}" id="delivery_option_{$id_address}_{$option@index}"/><span class="delivery_option_title"><span class="delivery_option_title bold">{$carrier.instance->name}</span>
											|	<span class="">
