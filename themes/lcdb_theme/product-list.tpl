...
{if isset($products)}
	{foreach from=$products item=product name=products}
		...
		<pre>{$features|print_r}</pre>
		<a class="buy" href="{$link->getPageLink('cart.php', true)}?add&amp;id_product={$product.id_product|intval}&amp;token={$static_token}">{l s='Buy'}</a>
		...
		<div class="block-product" itemscope itemtype="http://schema.org/Product">
			<div class="infos">
				...
				<div class="detail">
					...
					<span class="preservation"></span>
				</div>
				<div class="price reduction" itemscope itemtype="http://schema.org/Offer">
					<p class="reduction_rate"></p>
					<div class="full_price">
						<p class="price-piece"><span></span></p>
						<p class="price-kg"><span></span></p>
					</div>
					<div class="selling_price">
						<p class="price-piece" itemprop="price"></p>
						<p class="price-kg"></p>
					</div>
					{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}<span class="price" style="display: inline;">{if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span><br />{/if}
				</div>
