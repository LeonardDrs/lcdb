...
class AdminCarriersCEControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		$carrier_default_sort = array(
			array('value' => CarrierCE::SORT_BY_PRICE, 'name' => $this->l('Price')),
			array('value' => CarrierCE::SORT_BY_POSITION, 'name' => $this->l('Position'))
		);

		$carrier_default_order = array(
			array('value' => CarrierCE::SORT_BY_ASC, 'name' => $this->l('Ascending')),
			array('value' => CarrierCE::SORT_BY_DESC, 'name' => $this->l('Descending'))
		);
				...
				'fields' => array(
					'PS_CARRIER_DEFAULT' => array(
						...
						'list' => array_merge(
							...
							CarrierCE::getCarriers((int)Configuration::get('PS_LANG_DEFAULT'), true, false, false, null, CarrierCE::ALL_CARRIERS))
					),
			...
			'input' => array(
				...
				array(
					...
					'values' => array(
						array(
							'id' => 'billing_default',
							'value' => CarrierCE::SHIPPING_METHOD_DEFAULT,
							'label' => $this->l('Default behavior')
						),
						array(
							'id' => 'billing_price',
							'value' => CarrierCE::SHIPPING_METHOD_PRICE,
							'label' => $this->l('According to total price')
						),
						array(
							'id' => 'billing_weight',
							'value' => CarrierCE::SHIPPING_METHOD_WEIGHT,
			...
			if (!count($this->errors))
			{
				...
				if (isset($id) && !empty($id))
				{
					try {
						...
						{
							$current_carrier = new CarrierCE($id);
				...
				else
				{
					...
					{
						// Create new Carrier
						$carrier = new CarrierCE();
						$this->copyFromPost($carrier, $this->table);
						$carrier->position = CarrierCE::getHigherPosition() + 1;
						if ($carrier->add())
						{
	...
	public function ajaxProcessUpdatePositions()
	{
		...
		foreach ($positions as $position => $value)
		{
			...
			{
				if ($carrier = new CarrierCE((int)$pos[2]))
