		...
		<div class="bloc">
			...
			<form action="{$link->getModuleLink('cashondelivery', 'validation', [], true)}" method="post">
				...
				<p>
					...
					<br />{l s='You have chosen the cash on delivery method.' mod='cashondelivery'}
					<br /><br />
					{l s='The total amount of your order is' mod='cashondelivery'}
					<b><span id="amount_{$currencies.0.id_currency}" class="price">{convertPrice price=$total}</span>
					{if $use_taxes == 1}
					    {l s='(tax incl.)' mod='cashondelivery'}
					{/if}</b>
				</p>
				<p>{l s='Please confirm your order by clicking \'I confirm my order\'' mod='cashondelivery'}.</p>
				<div class="clearfix">
					<a href="{$link->getPageLink('order', true)}?step=3" class="red-button gradient left">{l s='Other payment methods' mod='cashondelivery'}</a>
					<input type="submit" name="submit" value="{l s='I confirm my order' mod='cashondelivery'}" class="green-button gradient right" />
				</div>
			</form>
		</div>
	</div>
